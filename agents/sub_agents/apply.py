    ...
    def location_decider_action(self, state: State) -> dict:
        ...
        try:
            ...
            try:
                ...
                logger.debug(result)
                return {"result": result, "outcome": "success","position_id":result.get("position_id","-1"),"relative_position": result.get("relative_position","NONE")}
            except:
                logger.error("Error parsing apply response")
            ...
            logger.debug(response)
            return {"result": {"error": f"Failed to handle LLM call: {str(e)}"}, "outcome": "error"}
